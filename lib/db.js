  ...
  async flush () { // The caller is responsible for ensuring this runs in a lock
    ...
    for (const info of coresAdding) {
      ...
      if (existing) {
        ...
        const prio = Math.max(info.priority || 0, existing.priority)
        const announce = info.announce || existing.announce
        ...
        await tx.insert('@blind-peer/cores', {
          ...
          updated: time,
          active: time,
          priority: Math.min(MAX_PRIO, prio),
          announce,
          referrer: existing.referrer,
          blocksCleared: existing.blocksCleared,
          bytesCleared: existing.blocksCleared
        })
      } else {
        await tx.insert('@blind-peer/cores', {
          key,
