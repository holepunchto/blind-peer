...
module.exports = class BlindPeerDB extends ReadyResource {
  ...
  async _open () {
    ...
    this._flushTimer = setTimeout(() => {
      this.flush().catch(safetyCatch)
      // runs in background, so no sane way to handle errors.
      // TODO: emit? (we do want to know when this fails)
    }, this.flushDelay)
  }
  ...
  async addCore (info) {
    ...
    if (this._tx.updates.size > this.maxPendingUpdates) this.flush().catch(safetyCatch)
  }
