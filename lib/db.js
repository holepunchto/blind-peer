...
module.exports = class BlindPeerDB extends ReadyResource {
  ...
  async flush() {
    ...
    for (const info of coresAdding) {
      ...
      const priority = Math.min(MAX_PRIO, info.priority || 0)
      if (existing) {
        if (!info.announce && priority <= existing.priority) continue // would be a downgrade, which we never want
        ...
        existing.blocksCleared = info.blocksCleared ?? existing.blocksCleared
        existing.bytesCleared = info.bytesCleared ?? existing.bytesCleared

        if (existing.bytesAllocated !== info.bytesAllocated) {
          bytesAllocated += info.bytesAllocated - existing.bytesAllocated
          existing.bytesAllocated = info.bytesAllocated
        }
    ...
    for (const core of coresUpdated.values()) {
      let skip = false
      for (const coreAdding of coresAdding) {
        if (b4a.equals(core.key, coreAdding.key)) {
          skip = true
        }
      }
      if (skip) {
